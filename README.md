# COVID-19 Early Warning System Project Overview
...
This project is presented in two parts. [The first part consists of the COVID-19 outbreak data analysis](https://github.com/salvir1/COVID-19-outbreak-data-analysis). The second part--contained in this repository--introduces the proposed COVID-19 early warning system. 
...
Let's start by assuming one wants to detect a 60% increase in prevalence within two days 85% of the time with alpha = 0.1. Without targeted sampling of the *high contact rate* subpopulation, one would need 4280 daily samples. With targeted sampling, the daily sample size would only need to be 1419.

There may be an added benefit to setting up a randomized surveillance testing program like the one above. Currently, it can take days for test results from labs to find their way into report datasets. The state of Washington advises that data may be revised for up to 10 days before they are confident that at least 90% of test results have been reported. Incomplete test results add yet more obscurity to today's statistics that rely on cases as a numerator. Statistics from a randomized surveillance testing program would not have the same integrity issues since the pertinent statistic is the positive test rate. The true mean for this random variable will not change  even if it's based on partial data.
